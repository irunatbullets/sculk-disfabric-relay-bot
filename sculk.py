...
def load_config():
    ...
    try:
        ...
        print(f"JSON5 decoding error {e}")
        return None
...
async def run_command(ctx, mc_user: str, command: str, reason: str = None):
    ...
    for channel_id in mc_server_channel_ids:
        ...
        try:
            ...
            def check(message):
                return (
                    "this player" in message.content.lower()
                    or "nothing changed" in message.content.lower()
                    or ("banned" in message.content.lower() and mc_user.lower())
                )
...
async def console(ctx, action: str = None, mc_user: str = None, *, reason: str = None):
    if not action:
        ...
        await ctx.send(
            "Available commands are `ban` and `pardon`. Usage is `!console ban username reason` or `!console pardon username`."
        )
        return
    if action == "ban":
...
async def channel(ctx, action: str = None, channel_id: str = None):
    if not action or not channel_id or not channel_id.isdigit():
        await ctx.send(
            "Invalid arguments. Usage is `!channel add channel_id` or `!channel remove channel_id`."
        )
        return
    ...
    if action == "add":
        if channel_id not in config["channels"]["mc_servers"]:
            ...
            with open("config.json5", "w") as f:
                f.write(
                    json5.dumps(config, indent=2)
                )  # Write the modified JSON5 content to the file
